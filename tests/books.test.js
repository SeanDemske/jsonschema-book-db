...
describe("POST /books", async function() {
    ...
    test("Required book title", async function() {
        const response = await request(app)
            .post("/books")
            .send({ year: 2000 });
...
describe("GET /books/:isbn", async function() {
    test("Gets a single book", async function() {
        const response = await request(app).get(`/books/${bookSample}`);
        expect(response.body.book).toHaveProperty("isbn");
        expect(response.body.book.isbn).toBe(bookSample);
    });
...
describe("DELETE /books/:id", async function() {
    test("Deletes a single book", async function() {
        const response = await request(app).delete(`/books/${bookSample}`);
